class Rekening:
    ...
    def transfer(self, penerima, jumlah):
        if self.saldo >= jumlah:
            ...
            print(f"Transfer sebesar {jumlah} dari rekening {self.no_rekening} ke rekening {penerima.no_rekening} berhasil.")
            print(f"Saldo pengirim ({self.nama}): {self.saldo}")
            print(f"Saldo penerima ({penerima.nama}): {penerima.saldo}")
        else:
            print("Saldo tidak mencukupi. Transaksi transfer tidak dapat dilakukan.")
...
def main():
    ...
    # Transaksi setor tunai, tarik tunai, dan transfer
